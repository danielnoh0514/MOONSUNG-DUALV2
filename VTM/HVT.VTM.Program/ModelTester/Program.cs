...
namespace HVT.VTM.Program
{
    public partial class Program
    {
        ...
        private void GetCDSValue(int threshold)
        {
            StringBuilder binaryString = new StringBuilder();
            StringBuilder resultString = new StringBuilder();

            int signalNumBoardExtension2 = 13;
            BoardExtension2.SevenSegment.SignalRead(signalNumBoardExtension2);
            BoardExtension2.SevenSegment.LEDs = BoardExtension2.SevenSegment.LogicLevel.ToList();
            int idxLed = 0;
            for (int i = 0; i < signalNumBoardExtension2; i++)
            {
                idxLed++;
            ...
            int signalNumBoardExtension1 = 6;
            BoardExtension1.SevenSegment.SignalRead(signalNumBoardExtension1);
            BoardExtension1.SevenSegment.LEDs = BoardExtension1.SevenSegment.LogicLevel.ToList();
            for (int i = 0; i < signalNumBoardExtension1; i++)
            {
                idxLed++;
        ...
        {
            MicrophoneViewer.Dispatcher.Invoke(() =>
            {
                MicrophoneViewer.StartRecording();
            });
        }
        ...
        private void StopRecording()
        {
            MicrophoneViewer.Dispatcher.Invoke(() =>
            {
                MicrophoneViewer.StopRecording();
            });
        }


        public bool PredictWithSlidingWindow(List<float> allData, int windowSize = 22500, int overlap = 2250)
        {
            bool result = false;
            ...
            for (int start = 0; start + windowSize <= allData.Count; start += step)
            {
                List<float> window = allData.GetRange(start, windowSize);
                result = MicrophoneViewer.AudioTester.Predict(window); // dùng hàm bạn đã viết
            ...
            if (int.TryParse(step.Condition2, out int scanTime))
            {
                ...
                while (DateTime.Now.Subtract(start).TotalMilliseconds < scanTime)
                {
                    ...
                    if (step.Condition1 == "LED")
                    {
                        ...
                        foreach (var led in CDSViewer.LEDs)
                        {
                            string substring = binaryString.ToString().Substring(led.Index - 1, 1);
                            led.State = (substring == "1") ? 1:0;
                            led.UpdateStateColor();
                        }
