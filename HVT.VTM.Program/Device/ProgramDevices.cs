...
namespace HVT.VTM.Program
{
    public partial class Program
    {
        ...
        //public BoardExtension BoardExtensionDUAL { get; set; }
        ...
        public Program()
        {
            ...
            System.System_Board.MachineIO.OnSubStartRequest += MachineIO_OnStartRequest;
            System.System_Board.MachineIO.OnCancleRequest += MachineIO_OnCancleRequest;
        }

        private void OnMainStartRequestHandler(object sender, EventArgs e)
        {
            System.System_Board.MachineIO.SS_DOWN = true;
        }
        private void OnMainCancelRequestHandler(object sender, EventArgs e)
        {
            System.System_Board.MachineIO.SS_DOWN = false;
        }
        public void NetToggleConnect()
        {
            if (AppSetting.Communication.Network.Use)
            {
                if (!AppSetting.Communication.Network.IsNetworkConnected)
                {
                    AppSetting.Communication.Network.Start();
                    AppSetting.Communication.Network.OnMainStartRequest += OnMainStartRequestHandler;
                    AppSetting.Communication.Network.OnMainCancelRequest += OnMainCancelRequestHandler;
                }
                else
                {
                    AppSetting.Communication.Network.Stop(); 
                    AppSetting.Communication.Network.OnMainStartRequest -= OnMainStartRequestHandler;
                    AppSetting.Communication.Network.OnMainCancelRequest -= OnMainCancelRequestHandler;
                }
            }
        ...
        {
            if (AppSetting.Communication.Network.IsMain)
            {
                AppSetting.Communication.Network.NotifyStatus(MachineStatus.CLUP);
            }
        ...
        private void MachineIO_OnStartRequest(object sender, EventArgs e)
        {
            ...
            //if (AppSetting.Communication.Network.IsMain)
            //{
            //    AppSetting.Communication.Network.NotifyStatus(MachineStatus.CLDOWN);
            //}


            if (AppSetting.Communication.MainPc)
            {
                System.System_Board.SendStartToSubPc();
            }
            ...
            if (!IsTestting && IsloadModel)
            {
                if (pageActive == PageActive.AutoPage)
                {
                    ...
                    System.System_Board.MachineIO.SubStart = false;
                    IsTestting = true;
                }
            }
        }
